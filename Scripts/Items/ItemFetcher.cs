...
public class ItemFetcher : MonoBehaviour
{
    ...
    public IEnumerator Init()
    {
        ...
        // Load all ball items from the Resources folder
        PokeballItem[] pokeballItems = Resources.LoadAll<PokeballItem>("Items/Pokeballs");
        // Load all recovery items from the Resources folder
        RecoveryItem[] recoveryItems = Resources.LoadAll<RecoveryItem>("Items/Recovery");
        EvolutionItem[] evolutionItems = Resources.LoadAll<EvolutionItem>("Items/EvolutionItems");

        // If there are already 55 TMs in the folder, return
        if ((tmAndHmItems.Length >= 55) && (pokeballItems.Length >= 4) && (recoveryItems.Length >= 20)&& (evolutionItems.Length >= 5))
        {
            Debug.Log("all items found. skipping API call");
            yield break;
        }
    ...
    public static IEnumerator CreateFromAPI(string itemName, MoveBase move = null)
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(url))
        {
            ...
            float price = response["cost"].AsInt;
            ...
            if (categoryName == "healing" || categoryName == "status-cures" || categoryName == "revival" || categoryName == "pp-recovery")
            {
                ...
                recoveryItem.SetItemValues(name, description, categoryName, null, price);
            ...
            else if (categoryName == "standard-balls" || categoryName == "special-balls")
            {
                ...
                pokeballItem.SetItemValues(name, description, categoryName, null, price);
            ...
            else if (categoryName == "evolution") // Category for evolutionary stones
            {
                EvolutionItem evolutionItem = ScriptableObject.CreateInstance<EvolutionItem>();
                description = "Evolves certain types of Pokemon";
                evolutionItem.SetItemValues(name, description, categoryName, null, price);
                newItem = evolutionItem;
    ...
    private ItemBase CreateAndSaveItem(ItemJSON itemJSON)
    {
        ...
        newItem.SetItemValues(formattedName, itemJSON.effect_entries[0].short_effect, itemJSON.category.name, null, itemJSON.cost);
    ...
    IEnumerator CreateTMItem(string name, MoveBase move, bool isHM)
    {
        ...
        int iconIndex = 315; // Replace this value with the actual index of the TM icon
        ...
        name = name.Replace("-", " ");
        float cost = 500f;
        newItem.SetItemValues(name, $"Teaches {name} to a Pokemon", "all-machines", iconSprite, cost);
    ...
    public void RenameItemsBasedOnIsHM()
    {
        ...
        TMItem[] allTMs = Resources.LoadAll<TMItem>("Items/TM&HM");

        // Sort items based on their name
        Array.Sort(allTMs, (x, y) => string.Compare(x.Move.Name, y.Move.Name));
        ...
        foreach (TMItem tm in allTMs)
        {
            ...
            if (tm.IsHM)
            {
                ...
                if (!oldPath.EndsWith(newPath.Substring(newPath.LastIndexOf("/"))))
                {
                    ...
                    AssetDatabase.MoveAsset(oldPath, newPath);
                     ...
                     // Update itemName to match the asset name
                    tm.SetItemName($"HM{hmCounter.ToString("00")}");
            ...
            else
            {
                ...
                if (!oldPath.EndsWith(newPath.Substring(newPath.LastIndexOf("/"))))
                {
                    ...
                    // Update itemName to match the asset name
                    tm.SetItemName($"TM{tmCounter.ToString("00")}");
                }
            }
    ...
    public static string GetFolderPathByCategory(string categoryName)
    {
        switch (categoryName)
        {
            ...
            case "all-machines":
                return "TM&HM";
            case "evolution":  // Category for evolutionary stones
                return "Evolution";  // Folder for evolutionary stones
            default:
                return "Other";
        }
    }
